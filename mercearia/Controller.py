...
class ControllerVenda:
  def cadastrarVenda(self,nomeProduto,vendedor,comprador,quantidadeVendida):
    ...
    for i in temp:
      with open("estoque.txt",'a') as arq:
        arq.writelines(i.nome + "|" + i.preco + "|" + i.categoria + "|" + str(i.quantidade))
        arq.writelines("\n")
  ...
  def relatorioProdutos(self):
    ...
    ordenados = sorted(produtos,key=lambda k: k['quantidade'], reverse = True)
  ...
  def mostrarVenda(self,dataInicio,dataTermino ):
    vendas = DaoVenda.ler()
    dataInicio1 = datetime.strftime(dataInicio,'%d/%m/%Y')
    dataTermino1 = datetime.strftime(dataTermino,'%d/%m/%Y')
...
class ControllerFornecedor:
  ...
  def alterarFornecedor(self, nomeAlterar, novoNome, novoCnpj, novoTelefone, novoCategoria):
    ...
    if len(est) > 0:
      ...
      if len(est) == 0:
        x = list(map(lambda x: Fornecedor(novoNome, novoCnpj, novoTelefone, novoCategoria) if(x.nome == nomeAlterar) else(x)))
      else:
  ...
  def removerFornecedor(self, nome):
    ...
    if len(est) > 0:
      for i in range(len(x)):
        ...
        print("O fornecedor que deseja excluir não existe")
        return None
...
class ControllerFuncionario:
  ...
  def removerFuncionario(sel,nome):
    x = DaoFuncionario.ler()
